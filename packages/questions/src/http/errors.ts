import { createHttpErrorHandler, DomainError, type ErrorMapper } from "@lemma/error";
import type { Context } from "hono";
import { QuestionsApplicationError } from "../application/errors.js";
import {
...
const applicationErrorMapper = {
  ...
  QUESTION_GENERATION_RUN_NOT_FOUND: { code: "QUESTION_GENERATION_RUN_NOT_FOUND", status: 404 },
  QUESTION_NOT_FOUND: { code: "QUESTION_NOT_FOUND", status: 404 },
  QUESTION_SET_NOT_FOUND: { code: "QUESTION_SET_NOT_FOUND", status: 404 },
  QUESTION_BLUEPRINT_NOT_FOUND: { code: "QUESTION_BLUEPRINT_NOT_FOUND", status: 404 },
  UNSUPPORTED_QUESTION_VALUE_EXPRESSION: { code: "UNSUPPORTED_QUESTION_VALUE_EXPRESSION", status: 400 },
  WORKBOOK_QUESTION_SOURCE_ERROR: { code: "WORKBOOK_QUESTION_SOURCE_ERROR", status: 502 },
} as const;
...
const domainErrorMapper = {
  ...
  INVALID_QUESTION_STATE_TRANSITION: { code: "QUESTION_STATE_CONFLICT", status: 409 },
...
export function handleQuestionsError(c: Context<QuestionsAppEnv>, error: unknown): Response {
...
function isQuestionsApplicationError(error: unknown): error is QuestionsApplicationErrorType {
